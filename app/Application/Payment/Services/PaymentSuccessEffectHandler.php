...
class PaymentSuccessEffectHandler
{
    ...
    public function handle(Payment $payment, bool $statusTransitioned): string
    {
        ...
        if ($this->shouldRunEtaEffect($payment)) {
            try {
            ...
            logSession(
                "paymentId {$payment->id} ETA yan etkisi daha once islenmis gorunuyor, tekrar cagrilmadi.",
                ['erp_status' => $payment->erp_status],
                'info',
                'payment_logs'
            );
        }
    ...
    private function shouldRunEtaEffect(Payment $payment): bool
    {
        $erpStatus = strtolower((string) ($payment->erp_status ?? ''));

        return !in_array($erpStatus, ['processing', 'sent'], true);
    }
}
