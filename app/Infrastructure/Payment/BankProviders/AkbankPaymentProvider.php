...
use App\Infrastructure\Payment\Clients\AkbankPosClient;

class AkbankPaymentProvider implements BankPaymentProviderInterface, BankPaymentStatusQueryableInterface, BankCodeAwarePaymentProviderInterface
{
    use MapsProviderFields;

    private const BANK_ID = 7;
    private const BANK_CODES = ['akbank'];
    ...
    {
        $response = (new AkbankPosClient($request->toProviderPayload()))->generatePaymentForm();
    ...
    public function resolveCallback(PaymentCallbackRequest $request): PaymentCallbackResult
    {
        ...
        $isSuccess = (($payload['responseCode'] ?? null) === 'VPS-0000');

        if ($isSuccess) {
            $providerReference = (string) ($payload['rrn'] ?? '');
        ...
        $message = sanitize_error_message($payload['hostMessage'] ?? 'Odeme Islemi Basarisiz.');
    ...
    {
        $result = AkbankPosClient::cancel($request->toProviderPayload());
    ...
    {
        $result = AkbankPosClient::refund($request->toProviderPayload());
    ...
    {
        $result = AkbankPosClient::queryStatus($request->toProviderPayload());

        if (!($result['success'] ?? false)) {
            return new PaymentStatusQueryResult(
                supported: true,
                status: 'unknown',
                message: (string) ($result['message'] ?? 'akbank_status_query_failed'),
