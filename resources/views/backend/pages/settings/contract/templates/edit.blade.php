    ...
    <div class="layout-px-spacing">
        <div class="row layout-top-spacing switch-outer-container">
            <div class="col-12 col-xl-9 layout-spacing">
                <form action="{{ route('admin.contracts.templates.update', $template->id) }}" method="POST"
                    id="template-form" data-ajax-form>
                    @csrf
                    @method('PATCH')
                        ...
                        <div class="widget-content widget-content-area">
                            ...
                            <div class="form-group">
                                ...
                                <select name="dealer_type" id="dealer_type" class="selectpicker w-100"
                                    ...
                                    <option value="dealer" {{ $template->dealer_type == 'dealer' ? 'selected' : '' }}>Bayi
                                    </option>
                                    <option value="subdealer" {{ $template->dealer_type == 'subdealer' ? 'selected' : '' }}>
                                        Alt Bayi
                                    </option>
                                    <option value="all" {{ $template->dealer_type == 'all' ? 'selected' : '' }}>
                                        Tümü</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label class="col-form-label" for="content">Sözleşme Metni (HTML destekli) <span
                                        class="text-danger">*</span></label>
                                <textarea class="form-control" id="content" name="content" data-editor="tinymce-6.7.0">
                                    {{ $template->content }}
                                </textarea>
                            </div>
                            <div class="mt-3">
