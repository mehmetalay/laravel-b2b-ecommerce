<template>
    <div>
        <ServerDataTable
            ...
            :per-page="50"
            column-toggle
            storage-key="attributes-table"
            loaded-event-component="attributes"
            empty-text="Özellik bulunamadı"
        ...
        >
            ...
            <template #cell-status="{ row }">
                <div class="custom-control custom-switch">
                    ...
                    >
                    <label class="custom-control-label" :for="`attribute-status-${row.id}`"></label>
                </div>
            </template>
            ...
            <template #cell-actions="{ row }">
                <a
                    v-if="typeof row.values_url === 'string' && row.values_url"
                    :href="String(row.values_url)"
                ...
                <a
                    href="javascript:;"
                    class="btn btn-secondary font-15 p-2 mr-1 attribute-sort-handle"
                    title="Sürükle bırak"
                >
        ...
        <FormModal
            v-model:show="show"
            :title="mode.value === 'edit' ? 'Özellik Düzenle' : 'Özellik Yeni'"
            :loading="loading"
            @close="onModalClose"
            @submit="submitForm"
        >
            <div class="form-group">
                <label for="attribute-name">Adı</label>
                <input
                    id="attribute-name"
                    v-model="data.name"
                    name="name"
            ...
            <div class="form-group mt-3">
                <label for="attribute-name-en">Adı (EN)</label>
                <input
            ...
            <div class="form-group mt-3">
                <div class="custom-control custom-checkbox">
                    <input id="attribute-status-modal" v-model="data.status" type="checkbox" class="custom-control-input">
                    <label class="custom-control-label" for="attribute-status-modal">Aktif</label>
                </div>
            </div>

            <div class="form-group mt-2">
                <div class="custom-control custom-checkbox">
                    <input id="attribute-show-filter" v-model="data.show_in_filter" type="checkbox" class="custom-control-input">
                    <label class="custom-control-label" for="attribute-show-filter">Filtrede göster</label>
                </div>
            </div>
        </FormModal>
    </div>
...
const props = defineProps<{
    attributeGroupId: number | string;
}>();
...
const endpoint = computed(() => (
    `/admin/api/attribute-groups/${encodeURIComponent(String(props.attributeGroupId))}/attributes`
));

const form = useModalForm<AttributeFormData>({
    id: null,
    name: '',
    name_en: '',
    status: true,
...
const { loading: bulkLoading, performBulkAction } = useBulkActions('attributes');
...
useServerDatatableSortable({
    ...
    rootSelector: '[data-vue="attributes-index"]',
    handleSelector: '.attribute-sort-handle',
    endpoint: () => `/aka/catalog/product-attributes/attribute-groups/${props.attributeGroupId}/attributes/sort`,
...
const runBulkAction = async (
    ...
    const success = await performBulkAction('/admin/api/attributes/bulk', action, ids, {
        rows,
...
const openCreateModal = () => {
    openCreate(`/aka/catalog/product-attributes/attribute-groups/${props.attributeGroupId}/attributes`, {
        id: null,
        name: '',
        name_en: '',
        status: true,
...
const openEditModal = async (row: RowData) => {
    ...
    await openEdit(String(row.edit_url), {
        submitEndpoint: `/aka/catalog/product-attributes/attribute-groups/${props.attributeGroupId}/attributes/${row.id}`,
        submitMethod: 'patch',
        mapData: (payload) => {
            const item = payload as Partial<AttributeFormData>;
            return {
                id: row.id,
